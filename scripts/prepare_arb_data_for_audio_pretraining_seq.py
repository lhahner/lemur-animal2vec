...
def iteration(file, args, id_generator, labels, channel_dict):
    # CUSTOMIZED CODE FROM FORK
    #labels = labels.sort_values("AudioFile").sort_values("StartRelative")
...
def main(args):
    ...
    if os.path.isfile(args.pickle_file):
        try:
            ...
            labels = None
            print("Could not load pickle file:", e, "\nContinuing without labels.")
    else:
        labels = None
    ...
    if os.path.isfile(args.stereo_file):
        try:
            ...
            channel_dict = None
            print("Could not load stereo file:", e, "\nContinuing without channel dict.")
    else:
        channel_dict = None
    ...
    if args.randomize_file_names:
        ...
        with open(csv_out, "w") as f:
            f.write("RandomizedName\tOriginalName\n")
